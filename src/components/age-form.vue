  ...
  >
    ...
    <Transition name="slide-fade">
      ...
      >
        ...
        <button
          @click="handleFirstStep(age, $event)"
          class="primary-btn"
          :class="{ 'disabled-btn': !age }"
          :disabled="!age"
        >
          Next
        </button>
    ...
    <Transition name="slide-fade">
      <form
        v-if="step === 'expected-age'"
        class="flex flex-col gap-5 justify-center items-center"
      >
        <div class="flex flex-col justify-center items-center">
          <label for="expected-age" class="input-label"
            >How old do you expect to live?</label
          >
          <input
            id="expected-age"
            type="number"
            ...
            max="150"
            autofocus
            ref="expectedAgeInputRef"
          />
        </div>
        <button
          @click="complete(expectedAge)"
          class="primary-btn"
          :class="{ 'disabled-btn': !expectedAge }"
          :disabled="!expectedAge"
        >
          Let's see!
        </button>
      </form>
